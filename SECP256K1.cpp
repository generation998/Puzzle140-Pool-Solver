...
void CheckAddress(Secp256K1 *T,std::string address,std::string privKeyStr) {
  ...
  case 'b':
  case 'B':
    if (address.size() >= 4 && (address[2] == '1' || address[2] == '1') && (address[3] == 'p' || address[3] == 'P'))
      type = P2TR;
    else
      type = BECH32;
    break;
...
void Secp256K1::Check() {
  ...
  Int one;
  one.SetInt32(1);
  Point gpub = ComputePublicKey(&one);
  uint8_t q1[32];
  bool tapOk = GetTaprootOutputKey(gpub, q1);
  std::string tapAddr = GetAddress(P2TR, true, gpub);
  int wver = -1;
  uint8_t wprog[40];
  size_t wlen = 0;
  int decOk = tapOk && tapAddr.size() > 0 && segwit_addr_decode(&wver, wprog, &wlen, "bc", tapAddr.c_str());
  printf("Check P2TR (key 1) %s :", tapAddr.c_str());
  PrintResult(decOk && wver == 1 && wlen == 32 && memcmp(q1, wprog, 32) == 0);

  Point tapP;
  tapP.x.SetBase16("cc8a4bc64d897bddc5fbc2f670f7a8ba0b386779106cf1223c6fc5d7cd6fc115");
  tapP.y = GetY(tapP.x, true);
  tapP.z.SetInt32(1);
  uint8_t q86[32];
  uint8_t exp86[32];
  Int exp;
  exp.SetBase16("a60869f0dbcf1dc659c9cecbaf8050135ea9e8cdc487053f1dc6880949dc684c");
  exp.Get32Bytes(exp86);
  bool tap86 = GetTaprootOutputKey(tapP, q86);
  std::string addr86 = GetAddressP2TR(q86);
  printf("Check P2TR BIP86 %s :", addr86.c_str());
  PrintResult(tap86 && memcmp(q86, exp86, 32) == 0 &&
...
static void TaggedHashTapTweak(const unsigned char *xonly, unsigned char *out32) {
  unsigned char tag[32];
  sha256((unsigned char *)"TapTweak", 8, tag);
  unsigned char buf[96];
  memcpy(buf, tag, 32);
  memcpy(buf + 32, tag, 32);
  memcpy(buf + 64, xonly, 32);
  sha256(buf, 96, out32);
}

bool Secp256K1::GetTaprootOutputKey(Point &pubKey, unsigned char *out32) {
  Point P(pubKey);
  P.Reduce();
  if (!P.y.IsEven())
    P.y.ModNeg();
  unsigned char xonly[32];
  P.x.Get32Bytes(xonly);
  unsigned char tweak[32];
  TaggedHashTapTweak(xonly, tweak);
  Int t;
  t.Set32Bytes(tweak);
  if (t.IsGreaterOrEqual(&order))
    return false;
  Point T = ComputePublicKey(&t);
  Point Q;
  if (P.equals(T))
    Q = DoubleDirect(P);
  else
    Q = AddDirect(P, T);
  Q.x.Get32Bytes(out32);
  return true;
}

std::string Secp256K1::GetAddressP2TR(unsigned char *program32) {
  char output[128];
  if (!segwit_addr_encode(output, "bc", 1, program32, 32))
    return "";
  return std::string(output);
}

void Secp256K1::GetP2WSH(Point &pubKey, unsigned char *out32) {
  unsigned char script[35];
  script[0] = 0x21;
  script[1] = pubKey.y.IsEven() ? 0x02 : 0x03;
  pubKey.x.Get32Bytes(script + 2);
  script[34] = 0xac;
  sha256(script, 35, out32);
}

std::string Secp256K1::GetAddressP2WSH(unsigned char *program32) {
  char output[128];
  if (!segwit_addr_encode(output, "bc", 0, program32, 32))
    return "";
  return std::string(output);
}
...
std::string Secp256K1::GetAddress(int type, bool compressed, Point &pubKey) {
  ...
  {
    uint8_t q[32];
    if (!GetTaprootOutputKey(pubKey, q))
      return "";
    return GetAddressP2TR(q);
  }
  break;
  ...
  {
    if (!compressed)
      return " P2WSH: Only compressed key ";
    uint8_t wsh[32];
    GetP2WSH(pubKey, wsh);
    return GetAddressP2WSH(wsh);
  }
  break;
